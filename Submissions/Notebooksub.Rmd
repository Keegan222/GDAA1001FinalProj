...
Two parameters will be needed for the models. One is the method in which the control data will be training the model. The method used is cross-validation. This method splits the data set randomly into groups and runs a series of tests that store results used to measure the metric of interest. We will repeat this for 20 unique tests sets. The metric of interest is the accuracy of the model.
...
This decision tree algorithm excels when the data consists of observations broken into groups. There are a lot of categorical variables with multiple levels of factors making this model an ideal choice at first glance.
...
The extra "tunegrid" parameter was necessary to complete the model, otherwise it never predicts the "Injury" factor level.
...
The accuracy of the model tends to fit around 55%. There is a strong purple color for the true positive of fatalities. It seems to have a more difficult time distinguishing between No ijury and Injury cases.
...
The K-nearest neighbors model generally performs well with numeric data or categorical data that has ordinal properties for the values. The prediction decisions are implemented fomr learning distances from nearest values and creating relations based on distances.
...
First observation of the results show a decrease in accuracy from our CART model. The data contains very few numeric values and the categorical variables intend to give a description of the feature and do not have a logical ordering.
...
Of the 675 observations in our validation set, the model was only able to predict the outcome correctly 289 times. It noticeably predicts that the accident has no injury when there is in fact an injury incorrectly more than it does correctly. 


SVM (Support Vector Machine):

Support Vector Machine models are very powerful and can handle linear and non-linear data. It also handles both binary and multiclass classifications well, which may be helpful for our three-level factor target variable.
...
A positive take-away from this model thus far is that there are noticeably smaller cases of false positives when the incident is a fatality but predicts no injury. Would be unfortunate for the model to say there's nothing to be concerned about when in fact someone is fatally injured on the scene. The accuracy however is still lower than our CART model. SVM's decision boundaries are made by creating hyperplanes that differentiate data into their classes. These hyperplanes can be sensitive to outliers, which are apparent in our variables that are dominantly outclassed by a single or few factor levels.
...
```{r}
multiModel <- multinom(InjurySeverity ~ ., data = training)
predictions <- predict(multiModel, validation)
...
The results of this model appear similar to but slightly outperformed by the CART model. Two major notes from the summary statistics are that the sensitivity of No Injury and specificity of fatality are generally higher in the multinomial logistic regression. This is illustrated below with the dark purple in the respective corners.
...
If the purpose of our model was to correctly predict No injury or fatality, this design would thrive. Hypothetically, if special equipment was required for accidents involving fatalities, you could erase some of the uncertainty that exists when the true outcome is something other than Injury.


Conclusion:

Use a lollipop graph to show importance of our predictor variables. The graph included many levels of our categorical factor, mkaing it quite messy. To clean this up, we only included factors which had an importance of at least greater than zero.

```{r}
importance3 <- varImp(cartModel)
imp3 <- importance3$importance

p3 <- imp3 %>% 
  mutate(Predictor = rownames(imp3)) %>% 
  pivot_longer(names_to = "InjurySeverity", values_to = "Importance", -Predictor) %>%
  filter(imp3>1)%>%
  ...
  ylab("Classification & Regression Tree - Before trim")+
  xlab("")
p3
```


This tell us some details about our predictors and which one stuck out significantly. With this knowledge about which variables are important in making predictions of our target variable, we can change the parameters of our model so attempt to increase the accuracy. 

```{r}
dataTrimmed <- data %>%
  select(c(InjurySeverity,VehicleType,Vehicles,UserType,Traffic,RoadConfig,PersonSex,CollisionType))

inTraining <- createDataPartition(dataTrimmed$InjurySeverity, p=0.75, list=FALSE)
training <- dataTrimmed[inTraining,]
validation <- dataTrimmed[-inTraining,]

cartModelTrimmed <- train(InjurySeverity~., data=training, method="rpart", metric=metric, trControl=control,
                   tuneGrid=data.frame(cp=0.01))
predictions <- predict(cartModelTrimmed, validation)

cmCartTrimmed <- confusionMatrix(predictions, as.factor(validation$InjurySeverity))
cmCartTrimmed

summary(cmCartTrimmed)
```

The accuracy of our trimmed data with application of the CART model is noticably higher by about 3% for this given example.

```{r}
importance3 <- varImp(cartModelTrimmed)
imp3 <- importance3$importance
...
p3 <- imp3 %>% 
  ...
  theme(
    ...
    panel.border = element_blank(),
    axis.ticks.y = element_blank())+
  ylab("Classification & Regression Tree - After trim")+
  xlab("")
p3
```

After further trimming the data, we run the model again and observe similar important variables. The consistency tells us that the variables are performing as they should.


The models:

```{r}
results <- resamples(list(cart=cartModel, knn=knnModel, svm=svmModel))
summary(results)
...
results_tidy <- results_df %>% 
  ...
  group_by(Resample, Model) %>%  # Include 'Resample' in grouping
  summarise(Mean_Accuracy = mean(Accuracy))

mean_acc <- results_df %>% 
  pivot_longer(names_to = "Model", values_to = "Accuracy", -Resample) %>% 
  ggplot(aes(x = fct_reorder(Model, -Accuracy), y = Accuracy)) +
...
The values of the x-axis are not very meaningful here, but the shape of the graph does display the level of performance of each model correctly. The CART outperforms most of the models with the exception of the multinomial logistic regression model not listed in this graph that also performs with around 55% accuracy.




Our model is most important and somewhat accurate when predicting fatalities, Which it does a decent job at distinguishing between fatality and the other two factors injury and no injury.  It is more likely to predict a fatality and be incorrect than to predict something else when the actual outcome of the accident is a fatality. This implication could be practically useful when dispatching emergency responders. If the accident has potential to fatally injure a passenger, a swift and confident call to send out emergency responders can save a life. The difference between No injury and Injury is sometimes less obvious with our results. 